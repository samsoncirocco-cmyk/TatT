...
The live artist graph holds **8,949 real scraped artists** (`data/cleanup-report.json`) across a national dataset — the earlier 100 synthetic seed artists have been deleted from the graph. `src/data/artists.json` (the 100-artist synthetic set) is only still read by the old-theme `/smart-match` and `/swipe` pages, which haven't been migrated or retired — see Open issues.

Enrichment (populating style tags/portfolio from each artist's scraped source pages) runs as a deterministic pipeline in the sibling `~/tatt-scraper` repo (`execution/enrich_artists.py`), not via Apify — a pilot batch (~212 artists) has run, the full sweep has not launched yet. See `TODO.md` for status. The overnight crew (`CREW.md`) works this repo's issue queue in two lanes — `pr-only` (code tickets, PRs only, no paid/data jobs) and `autonomous` (may also merge and run capped data/spend jobs) — currently in `pr-only` mode.
...
- **`/smart-match` and `/swipe` (old-theme) still read the 100-artist synthetic seed** (`src/data/artists.json`, via `ArtistsContent.jsx`/`ArtistProfileContent.jsx`), not the real 8,949-artist scraped dataset. `/matches`, `/artists`, and `/book` are all live on the graph; these two orphaned old-theme pages are the last synthetic-data holdouts (tracked in issue #59).
