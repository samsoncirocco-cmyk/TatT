...
- **Empty state for `/bookings`** — the parallel session built this
  page; I did not verify whether it has an empty state.
...
- Footer on app pages.
- Live design count + favorite count in top nav — partial: the nav
  shows favorite count via a `♥ N` chip from the parallel session's
  commit; per-link badges (designs/bookings) are not wired.
