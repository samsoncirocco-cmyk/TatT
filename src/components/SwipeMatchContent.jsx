...
function SwipeMatchContent() {
  ...
  useEffect(() => {
    try {
      ...
      if (!router.query || !router.query.preferences) {
      ...
      const preferences = router.query?.preferences;
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 overflow-hidden pt-20">
      ...
      {currentIndex < 0 && (
        <div className="text-center mt-12 animate-fade-in font-bold relative z-10 glass-panel p-8 rounded-3xl">
          ...
          <Button
            onClick={() => router.push({ pathname: '/Artists', query: { likedIds: likedArtists.map(a => a.id).join(',') } })}
            variant="primary"
            size="lg"
            className="w-full"
          >
...
export default SwipeMatchContent;
