...
describe("artist profile — Instagram embed tier (TAT-40)", () => {
  ...
  it("flag on, claimed artist: hosted licensed images, no embeds", async () => {
    vi.stubEnv("ENABLE_IG_EMBEDS", "true");
    const hosted = "https://storage.googleapis.com/tatt-pro-assets/artists/artist_1/0.jpg";
  ...
  it("kill switch off + embeds on: unclaimed profile shows embeds and zero hosted images", async () => {
    ...
    const { container } = await renderProfile(
      graphRow({
        portfolioImages: ["https://storage.googleapis.com/tatt-pro-assets/artists/artist_1/0.jpg"],
      }),
    );
    expect(container.querySelectorAll("blockquote.instagram-media")).toHaveLength(4);
    expect(container.querySelector("img")).toBeNull();
  });

  it("kill switch off + embeds off (or no permalinks): the existing monogram/stub state", async () => {
    vi.stubEnv("SHOW_UNCLAIMED_PORTFOLIOS", "false");
    ...
    expect(container.querySelector("img")).toBeNull();
    expect(container.querySelector("blockquote.instagram-media")).toBeNull();
    // The hero monogram is the fallback face of the profile.
    ...
    vi.stubEnv("ENABLE_IG_EMBEDS", "true");
    const second = await renderProfile(graphRow({ portfolioPermalinks: undefined }));
    expect(second.container.querySelector("blockquote.instagram-media")).toBeNull();
    expect(second.container.textContent).toContain("SI");
  });
});
