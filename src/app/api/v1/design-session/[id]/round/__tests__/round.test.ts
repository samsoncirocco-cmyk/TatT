// Seam tests for POST /api/v1/design-session/[id]/round (ADR-0049): the
// designSession service is mocked at its public entry point; these pin the
// route's policy — auth gate, generation rate bucket, budget, the ONE
// credit reserved per round and released on failure or downgrade, the
// decided failure copy, and the { success, session, round } shape.
...
const URL = 'http://localhost/api/v1/design-session/sess-1/round';
const RESERVATION = { id: 'res-1', source: 'free', freeRemaining: 24, paidRemaining: 0 };

/** A delivered round-2 result, the service's RefineRoundResult shape. */
function roundResult(overrides: Partial<{ downgraded: boolean; downgradeReason: string }> = {}) {
  const round = {
    round: 2,
...
describe('POST /api/v1/design-session/[id]/round route adapter', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('reserves ONE credit, runs the round, and returns session + round', async () => {
    refineRoundMock.mockResolvedValueOnce(roundResult());
  ...
  it('releases the credit and answers with the decided copy when the round dies', async () => {
    ...
    const json = await res.json();
    expect(json.error).toBe("That round didn't take — your credit is back. Run it again?");
    expect(json.code).toBe('ROUND_FAILED');
    expect(json.creditReleased).toBe(true);
  });

  // ADR-0048's loud downgrade: delivered off the pinned lane → refunded and
  // flagged, never silent.
  it('releases the credit and flags the response when the round downgraded', async () => {
    refineRoundMock.mockResolvedValueOnce(
      roundResult({ downgraded: true, downgradeReason: 'REPLICATE_ERROR' })
    );
  ...
  it('still returns the downgraded round when the release itself fails', async () => {
    refineRoundMock.mockResolvedValueOnce(roundResult({ downgraded: true }));
    releaseGenerationCreditMock.mockRejectedValueOnce(new Error('firestore down'));

    const res = await POST(makeRequest(URL, {}), routeParams('sess-1'));

    expect(res.status).toBe(200);
    expect((await res.json()).creditReleased).toBe(false);
  });
