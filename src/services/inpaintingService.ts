...
export interface InpaintingModelConfig {
  version: string;
  cost: number;
}

export interface InpaintTattooParams {
  imageUrl: string;
...
export interface InpaintingPrediction {
  id: string;
...
export interface InpaintingCostEstimate {
  perSecond: number;
  estimated: number;
  formatted: string;
}

export interface InpaintingAPIError {
  error?: string;
  detail?: string;
}
...
export const INPAINTING_MODEL: InpaintingModelConfig = {
...
}: InpaintTattooParams): Promise<string> {
  try {
    ...
    if (!createResponse.ok) {
      const errorData: InpaintingAPIError = await createResponse.json();
      throw new Error(errorData.error || errorData.detail || 'Failed to start inpainting');
    }

    const prediction: InpaintingPrediction = await createResponse.json();
    ...
    if (!outputImage || typeof outputImage !== 'string') {
      throw new Error('Invalid inpainting output');
    }
 ...
 */
export function getInpaintingCost(numInferenceSteps: number = 50): InpaintingCostEstimate {
