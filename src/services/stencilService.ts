import { PAPER_SIZES, validateDimensions, validateDPI, DEFAULT_DPI } from '../utils/stencilCalibration.js';
import { createStencilPDF } from '../utils/pdfGenerator.js';

// ============================================================================
// Types
// ============================================================================

export interface StencilSize {
  name: string;
  inches: number;
  pixels: number;
  description: string;
...
export interface GenerationOptions extends ConversionOptions {
...
export interface ProcessedStencil {
  dataUrl: string;
  size: StencilSize;
}
...
export const STENCIL_SIZES: Record<string, StencilSize> = {
  small: {
    name: 'Small (4")',
    inches: 4,
...
const DEFAULT_STENCIL_SETTINGS: Required<StencilSettings> = {
...
function resolvePaperDimensions(paperSizeKey: string, customPaperDimensions: PaperDimensions = {}): Required<PaperDimensions> {
  if (paperSizeKey === 'custom') {
    ...
    return {
      ...
      unit: 'inches',
      width: validated.widthInches,
      height: validated.heightInches
    };
  }
...
function enforcePaperFit(designDimensions: DesignDimensions, paperDimensions: Required<PaperDimensions>): void {
...
function buildExportMetadata(userMetadata: ExportMetadata = {}, context: { designDimensions?: DesignDimensions; paperSizeKey?: string; format?: string } = {}): ExportMetadata {
  const designDimensions = context.designDimensions || {} as DesignDimensions;

  const base: ExportMetadata = {
...
async function processStencilImage(
  imageUrl: string,
  sizeKey: string,
  settings: StencilSettings,
  mode: string,
  onProgress?: (progress: number) => void
): Promise<ProcessedStencil> {
 ...
 * @param settings - Stencil conversion settings
 * @param options - Additional options
 * @returns Data URL of stencil image
 */
export async function convertToStencil(
  imageUrl: string,
  settings: StencilSettings = {},
  ...
  const stencilOptions: Required<StencilSettings> = { ...DEFAULT_STENCIL_SETTINGS, ...settings };
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = async () => {
      try {
        ...
        const processChunk = async (startPixel: number) => {
          ...
          if (shouldChunk && endPixel < totalPixels) {
            await new Promise<void>((resolveChunk) => setTimeout(resolveChunk, 0));
          }
        };
        ...
        reject(new Error(`Failed to process stencil: ${(error as Error).message}`));
      }
    };
 ...
 * @param sizeKey - Key from STENCIL_SIZES
 * @returns Data URL of resized image
 */
export async function resizeToStencilSize(imageUrl: string, sizeKey: string): Promise<string> {
  // Validate inputs
  if (!imageUrl) {
    throw new Error('Image URL is required');
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        reject(new Error(`Failed to resize image: ${(error as Error).message}`));
      }
    };

    img.onerror = () => {
      reject(new Error('Failed to load image. Please check the image URL.'));
    };
...
): Promise<string | PDFExportResult> {
  const {
    exportFormat = 'png',
    onProgress,
    metadata,
...
export async function generateStencilPDF(
  ...
  const progress = typeof onProgress === 'function'
    ? (value: number) => onProgress(Number(Math.min(Math.max(value, 0), 1).toFixed(3)))
    : null;
  ...
  const designDimensions: DesignDimensions = {
 ...
 * @param data - Stencil data URL or Blob
 * @param filename - Download filename
 */
export function downloadStencil(data: string | Blob, filename: string = 'tattoo-stencil.png'): void {
