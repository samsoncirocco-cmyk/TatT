...
export type StencilSizeKey = 'small' | 'medium' | 'large' | 'xlarge';
export type ProcessingMode = 'threshold' | 'edge';
...
export interface StencilPDFResult {
  blob: Blob;
  metadata: Record<string, any>;
  filename: string;
}
...
export const STENCIL_SIZES: Record<StencilSizeKey, StencilSize> = {
  small: {
    name: 'Small (4")',
    inches: 4,
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = async () => {
      try {
        ...
        reject(new Error(`Failed to process stencil: ${error instanceof Error ? error.message : 'Unknown error'}`));
      }
    };
 ...
 */
export async function resizeToStencilSize(
  imageUrl: string,
  sizeKey: StencilSizeKey
): Promise<string> {
  // Validate inputs
  if (!imageUrl) {
    throw new Error('Image URL is required');
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        reject(new Error(`Failed to resize image: ${error instanceof Error ? error.message : 'Unknown error'}`));
      }
    };
...
export async function generateStencil(
  imageUrl: string,
  sizeKey: StencilSizeKey = 'medium',
  settings: StencilSettings = {},
  mode: ProcessingMode = 'threshold',
  options: StencilOptions = {}
): Promise<string | StencilPDFResult> {
  const {
    exportFormat = 'png',
    onProgress,
    metadata,
...
function resolvePaperDimensions(
  paperSizeKey: string,
  customPaperDimensions: StencilOptions['customPaperDimensions'] = {}
): PaperDimensions {
  ...
  const preset = PAPER_SIZES[paperSizeKey as keyof typeof PAPER_SIZES] || PAPER_SIZES.letter;
...
function enforcePaperFit(designDimensions: DesignDimensions, paperDimensions: PaperDimensions): void {
 ...
 */
export const STYLE_PRESETS: Record<string, StencilPreset> = {
  traditional: {
    name: 'Traditional',
    threshold: 140,
    contrast: 1.3,
