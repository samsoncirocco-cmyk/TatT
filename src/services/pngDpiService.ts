...
const PIXELS_PER_INCH_TO_METER = 39.3701;
 ...
 */
function createPhysChunk(pixelsPerMeterX: number, pixelsPerMeterY: number): Uint8Array {
 ...
 */
function hasPhysChunk(uint8Array: Uint8Array): boolean {
 ...
 */
function removePhysChunk(uint8Array: Uint8Array): Uint8Array {
  let offset = 8;
 ...
 *
 * @param pngBlob - Input PNG blob
 * @returns PNG blob with 300 DPI metadata
 */
export async function add300DpiMetadata(pngBlob: Blob): Promise<Blob> {
  try {
    ...
    const pixelsPerMeter = Math.round(300 * PIXELS_PER_INCH_TO_METER);
 ...
 */
export async function getDpiFromPng(pngBlob: Blob): Promise<DpiInfo | null> {
  try {
    const arrayBuffer = await pngBlob.arrayBuffer();
    const uint8Array = new Uint8Array(arrayBuffer);
    ...
    while (offset < uint8Array.length - 8) {
      ...
      if (type === 'pHYs') {
        ...
        if (unit === 1) {
          ...
          const dpiX = Math.round(pixelsPerMeterX / PIXELS_PER_INCH_TO_METER);
          const dpiY = Math.round(pixelsPerMeterY / PIXELS_PER_INCH_TO_METER);
