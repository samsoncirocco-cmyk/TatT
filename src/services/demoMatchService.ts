 ...
 */
function calculateStyleMatch(artist: MatchedArtist, design: DesignInput): number {
 ...
 */
function calculateLocationScore(artist: MatchedArtist, preferences: UserPreferences): number {
 ...
 */
function calculateBudgetScore(artist: MatchedArtist, preferences: UserPreferences): number {
 ...
 */
function getMockMatches(design: DesignInput): MatchedArtist[] {
  return [
    {
      id: 'mock-1',
      name: 'Alex Rivera',
 ...
 * @returns Top 5 matching artists
 */
export async function findMatchingArtistsForDemo(
  design: DesignInput,
  ...
  try {
    ...
    // Score each artist
    const scoredArtists: MatchedArtist[] = graphResults.map(artist => {
      // Calculate scores
